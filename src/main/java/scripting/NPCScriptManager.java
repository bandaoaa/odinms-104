...
public class NPCScriptManager extends AbstractScriptManager {
    ...
    public void start(MapleClient c, int npc, int npcMode) {
        ...
        try {
            if ((!this.cms.containsKey(c)) && (c.canClickNPC())) {
               if (c.getPlayer().isGM()) {
                    c.getPlayer().dropMessage(5, "您已经建立与NPC: " + npc + "_"+npcMode+" 的对话。");
                }
                Invocable iv;
                if (npcMode == 0) {
                ...
                NPCConversationManager cm = new NPCConversationManager(c, npc, -1, -1, (byte) -1, npcMode == 0 ? 0 : npcMode, iv);
                if ((iv == null) || (getInstance() == null)) {
                    if (iv == null) {
                        cm.sendOk("你好，我暂时没有功能。如果你有好的意见可以提交给管理员。.\r\n我的ID编号: #r " + npc + "_"+npcMode+" #k.");
                    }
                    dispose(c);
                    return;
                }
    ...
    public void dispose(MapleClient c) {
        ...
        if (npccm != null) {
            ...
            if (npccm.getItem() > 0) {
                c.removeScriptEngine("scripts/item/" + npccm.getItem() + ".js");
            }
        }

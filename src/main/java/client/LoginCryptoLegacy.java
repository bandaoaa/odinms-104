...
public class LoginCryptoLegacy {
    ...
    private static String myCrypt(String password, String seed)
        ...
        try {
            ...
            digester.update(new StringBuilder().append(salt).append(password).toString().getBytes("iso-8859-1"), 0, new StringBuilder().append(salt).append(password).toString().length());
            ...
            out = new StringBuilder().append(out).append(encode64(sha1Hash)).toString();
        } catch (NoSuchAlgorithmException | UnsupportedEncodingException Ex) {
            log.error(new StringBuilder().append("Error hashing password.").append(Ex).toString());
        }
        if (out == null) {

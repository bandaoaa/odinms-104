...
public class Battler implements Serializable {
    ...
    public Battler(int level, int exp, int charId, int monsterId, String name, BattleConstants.PokemonNature nature, int itemId, byte gender, byte hpIV, byte atkIV, byte defIV, byte spatkIV, byte spdefIV, byte speedIV, byte evaIV, byte accIV, byte ability) {
        ...
        for (BattleConstants.PokemonStat stat : BattleConstants.PokemonStat.values()) {
            ...
            this.mods.put(stat, new Pair(Byte.valueOf(theIV), Double.valueOf(1.0D)));
        }
    ...
    public byte getIV(BattleConstants.PokemonStat stat) {
        return ((Byte) ((Pair) this.mods.get(stat)).left).byteValue();
    }
    ...
    public void setStatus(MonsterStatusEffect mse) {
        ...
        if ((this.ability == PokemonAbility.Immunity) && (stat == MonsterStatus.中毒)) {
            return;
        }
        if ((this.ability == PokemonAbility.Insomnia) && (stat == MonsterStatus.恐慌)) {
            return;
        }
        if ((this.ability == PokemonAbility.Limber) && (stat == MonsterStatus.影网)) {
            return;
        }
        if ((this.ability == PokemonAbility.MagmaArmor) && (stat == MonsterStatus.结冰)) {
            return;
        }
        if ((this.ability == PokemonAbility.OwnTempo) && (stat == MonsterStatus.挑衅)) {
            return;
        }
        if ((this.ability == PokemonAbility.WaterVeil) && (stat == MonsterStatus.烈焰喷射)) {
            return;
        }
        if (this.status != null) {
            return;
        ...
        if (this.ability == PokemonAbility.Guts) {
        ...
        } else if (this.ability == PokemonAbility.MarvelScale) {
            for (int zz = 0; zz < getStatusTurns() - 1; zz++) {
                setMod(BattleConstants.PokemonStat.DEF, increaseMod(getMod(BattleConstants.PokemonStat.DEF)));
            }
        } else if (this.ability == PokemonAbility.QuickFeet) {
            for (int zz = 0; zz < getStatusTurns() - 1; zz++) {
                setMod(BattleConstants.PokemonStat.SPEED, increaseMod(getMod(BattleConstants.PokemonStat.SPEED)));
            }
        } else if ((stat == MonsterStatus.速度) || (stat == MonsterStatus.影网)) {
            for (int zz = 0; zz < getStatusTurns() - 1; zz++) {
                setMod(BattleConstants.PokemonStat.SPEED, decreaseMod(getMod(BattleConstants.PokemonStat.SPEED)));
            }
        } else if (stat == MonsterStatus.烈焰喷射) {
            for (int zz = 0; zz < getStatusTurns() - 1; zz++) {
                setMod(BattleConstants.PokemonStat.ATK, decreaseMod(getMod(BattleConstants.PokemonStat.ATK)));
            }
            if (this.ability == PokemonAbility.FlareBoost) {
                setMod(BattleConstants.PokemonStat.SPATK, increaseMod(getMod(BattleConstants.PokemonStat.SPATK)));
            }
        } else if (stat == MonsterStatus.鬼刻符) {
            for (int zz = 0; zz < getStatusTurns() - 1; zz++) {
                setMod(BattleConstants.PokemonStat.SPATK, decreaseMod(getMod(BattleConstants.PokemonStat.SPATK)));
            }
            ...
            if (this.ability == PokemonAbility.TangledFeet) {
                setMod(BattleConstants.PokemonStat.SPEED, increaseMod(getMod(BattleConstants.PokemonStat.SPEED)));
            }
        } else if ((stat == MonsterStatus.中毒)
                && (this.ability == PokemonAbility.Unaware)) {
            setMod(BattleConstants.PokemonStat.ATK, increaseMod(getMod(BattleConstants.PokemonStat.ATK)));
        }
    }
    ...
    public int getNextExp() {
        ...
        return (int) Math.ceil(family.type.value + family.type.value * level / 10.0D) * level * level;
    }

    public BattleConstants.Evolution getEvolutionType() {
        for (int i = 0; i < family.evolutions.size(); i++) {
            if (this.monsterId == (family.evolutions.get(i)).intValue()) {
                if (i == family.evolutions.size() - 1) {
                    return Evolution.NONE;
                }
                if (i != family.evolutions.size() - 2) {
    ...
    public double getMod(BattleConstants.PokemonStat stat) {
        return ((Double) ((Pair) this.mods.get(stat)).right).doubleValue() * (((Byte) ((Pair) this.mods.get(stat)).left).byteValue() / 250.0D + 0.8D) * (this.nature.inc == stat ? 1.1D : 1.0D) * (this.nature.dec == stat ? 0.9D : 1.0D);
    }
    ...
    public int getAverageIV() {
        ...
        for (Map.Entry stat : this.mods.entrySet()) {
            if (stat.getKey() != BattleConstants.PokemonStat.NONE) {
                total += ((Byte) ((Pair) stat.getValue()).left).byteValue();
                num++;
            }
        }
        return total / num;
    ...
    public String getIVString() {
        ...
        for (Entry<BattleConstants.PokemonStat, Pair<Byte, Double>> stat : mods.entrySet()) {
            if (stat.getKey() != BattleConstants.PokemonStat.NONE) {
                ss.append("#b#e").append(StringUtil.makeEnumHumanReadable(((PokemonStat) stat.getKey()).name()).toUpperCase()).append("#n#k - ").append(getIVString(((Byte) ((Pair) stat.getValue()).left).byteValue())).append("\r\n");
            }
        }
    ...
    public String getElementString() {
        return new StringBuilder().append(StringUtil.makeEnumHumanReadable(elements[0].name()).toUpperCase()).append(elements[1] == BattleConstants.PokemonElement.None ? "" : new StringBuilder().append("/").append(StringUtil.makeEnumHumanReadable(elements[1].name()).toUpperCase()).toString()).toString();
    }

    public String getNatureString() {
        return new StringBuilder().append(StringUtil.makeEnumHumanReadable(nature.name())).append("(+").append(StringUtil.makeEnumHumanReadable(nature.inc.name())).append("/-").append(StringUtil.makeEnumHumanReadable(this.nature.dec.name())).append(")").toString();
    }
    ...
    public int getElementSize() {
        ...
        for (int i = 0; i < this.elements.length; i++) {
            if (this.elements[i] != PokemonElement.None) {
                ret++;
            }
        }
        return ret;
    ...
    private void calculateFamily() {
        ...
        for (Entry<Element, ElementalEffectiveness> e : this.stats.getElements().entrySet()) {
        ...
        if (this.elements[0] == PokemonElement.None) {
            pushElement(PokemonElement.Normal);
        }
        resetStats();
        List<PokemonMob> ourFamilies = new ArrayList();
        for (PokemonMob mob : PokemonMob.values()) {
            if (mob.evolutions.contains(Integer.valueOf(this.monsterId))) {
                ourFamilies.add(mob);
            }
        }
        if (ourFamilies.size() > 0) {
            this.family = (ourFamilies.get(Randomizer.nextInt(ourFamilies.size())));
        }
        if (this.family != null) {
    ...
    public void addMonsterId(int uniqueId) {
        if (!this.damagedChars.contains(Integer.valueOf(uniqueId))) {
            this.damagedChars.add(Integer.valueOf(uniqueId));
        }
    }

    public void removeMonsterId(int uniqueId) {
        for (int i = 0; i < this.damagedChars.size(); i++) {
            if ((this.damagedChars.size() > i) && (((Integer) this.damagedChars.get(i)).intValue() == uniqueId)) {
    ...
    public void evolve(boolean skipCheck, MapleCharacter chr) {
        ...
        List<Integer> evo = this.family.evolutions;
        int ourIndex = -1;
        ...
        if ((ourIndex >= 0) && (evo.size() > ourIndex + 1)) {
            ...
            if ((this.level >= next.getStats().getLevel()) || (skipCheck)) {
                ...
                chr.getMonsterBook().monsterCaught(chr.getClient(), next.getId(), this.stats.getName());
            }
        }
    }

    public List<Integer> getDamaged() {
        return this.damagedChars;
    }
}
...
public class MapleServerHandler extends ChannelInboundHandlerAdapter implements MapleServerHandlerMBean {
    ...
    private static FileWriter isLoggedIP(ChannelHandlerContext ctx) {
        ...
        return (FileWriter) logIPMap.get(realIP);
    }

    public static void addIP(String theIP) {
        try {
            FileWriter fw = new FileWriter(new File(new StringBuilder().append("PacketLog_").append(theIP).append(".txt").toString()), true);
    ...
    public static void log(String packet, String op, MapleClient c, ChannelHandlerContext ctx) {
        try {
            ...
            if (Packet_Log.size() == Log_Size) {
                logged = (LoggedPacket) Packet_Log.remove(0);
            }

            if (logged == null) {
                logged = new LoggedPacket(packet, op, ctx.channel().remoteAddress().toString(), c == null ? -1 : c.getAccID(), FileoutputUtil.CurrentReadable_Time(), (c == null) || (c.getAccountName() == null) ? "[Null]" : c.getAccountName(), (c == null) || (c.getPlayer() == null) || (c.getPlayer().getName() == null) ? "[Null]" : c.getPlayer().getName(), (c == null) || (c.getPlayer() == null) || (c.getPlayer().getMap() == null) ? "[Null]" : String.valueOf(c.getPlayer().getMapId()), (c == null) || (NPCScriptManager.getInstance().getCM(c) == null) ? "[Null]" : String.valueOf(NPCScriptManager.getInstance().getCM(c).getNpc()));
            } else {
                logged.setInfo(packet, op, ctx.channel().remoteAddress().toString(), c == null ? -1 : c.getAccID(), FileoutputUtil.CurrentReadable_Time(), (c == null) || (c.getAccountName() == null) ? "[Null]" : c.getAccountName(), (c == null) || (c.getPlayer() == null) || (c.getPlayer().getName() == null) ? "[Null]" : c.getPlayer().getName(), (c == null) || (c.getPlayer() == null) || (c.getPlayer().getMap() == null) ? "[Null]" : String.valueOf(c.getPlayer().getMapId()), (c == null) || (NPCScriptManager.getInstance().getCM(c) == null) ? "[Null]" : String.valueOf(NPCScriptManager.getInstance().getCM(c).getNpc()));
            }
    ...
    public void writeLog(boolean crash) {
        ...
        try {
            FileWriter fw = new FileWriter(new File(new StringBuilder().append(Packet_Log_Output).append(Packet_Log_Index).append(crash ? "_DC.txt" : ".txt").toString()), true);
            String nl = System.getProperty("line.separator");
            for (LoggedPacket loggedPacket : Packet_Log) {
                fw.write(loggedPacket.toString());
                fw.write(nl);
            }
            String logString = new StringBuilder().append("Log has been written at ").append(lastDC).append(" [").append(FileoutputUtil.CurrentReadable_Time()).append("] - ").append(numDC).append(" have disconnected, within ").append(System.currentTimeMillis() - lastDC).append(" milliseconds. (").append(System.currentTimeMillis()).append(")").toString();
    ...
    public static void initiate() {
        ...
        if (Log_Packets) {
            for (int i = 1; i <= Packet_Log_Size; i++) {
                if ((!new File(new StringBuilder().append(Packet_Log_Output).append(i).append(".txt").toString()).exists()) && (!new File(new StringBuilder().append(Packet_Log_Output).append(i).append("_DC.txt").toString()).exists())) {
                    Packet_Log_Index = i;
                    break;
                }
            }
    ...
    public void channelActive(final ChannelHandlerContext ctx) throws Exception {
        ...
        //FileoutputUtil.log("log\\Address.log", address);//登陆IP日志记录
        ...
        if ((ServerConstants.isVpn(address)) || (!Start.instance.isIvCheck())) {
            ...
            ctx.writeAndFlush(LoginPacket.getHello(ServerConstants.MAPLE_VERSION, ivSend, ivRecv, false));
        }
        ...
        if (this.channel > -1) {
            sb.append("[连接频道服务器] 频道: ").append(this.channel).append(" : ");
        } else if (this.cs) {
            sb.append("[连接商城服务器] ");
        } else {
            sb.append("[连接登录服务器] ");
        }
        sb.append("玩家IP ").append(address);
        ...
        if (fw != null) {
            if (this.channel > -1) {
                ...
                fw.write(new StringBuilder().append("=== Logged Into Channel ").append(this.channel).append(" ===").toString());
                fw.write(nl);
            } else if (this.cs) {
                client.setMonitored(true);
    ...
    public void channelInactive(final ChannelHandlerContext ctx) throws Exception {
        ...
        if (client != null) {
            ...
            if (state != 3) {
                log(new StringBuilder().append("Data: ").append(numDC).toString(), "CLOSED", client, ctx);
    ...
    public void channelRead(final ChannelHandlerContext ctx, final Object message) {
        ...
        LittleEndianAccessor slea = new LittleEndianAccessor(new ByteArrayByteStream((byte[]) (byte[]) message));
...
//                    }
                    if ((c.getPlayer() != null) && (c.isMonitored()) && (!blocked.contains(recv))) {
                        FileWriter fw = new FileWriter(new File(new StringBuilder().append("MonitorLogs/").append(c.getPlayer().getName()).append("_log.txt").toString()), true);
                        fw.write(new StringBuilder().append(String.valueOf(recv)).append(" (").append(Integer.toHexString(header_num)).append(") Handled: \r\n").append(slea.toString()).append("\r\n").toString());
                        fw.flush();
                        fw.close();
                    }
                    ...
                    if ((fw != null) && (!blocked.contains(recv))) {
                        if ((recv == RecvPacketOpcode.PLAYER_LOGGEDIN) && (c != null)) {
                            fw.write(new StringBuilder().append(">> [AccountName: ").append(c.getAccountName() == null ? "null" : c.getAccountName()).append("] | [IGN: ").append((c.getPlayer() == null) || (c.getPlayer().getName() == null) ? "null" : c.getPlayer().getName()).append("] | [Time: ").append(FileoutputUtil.CurrentReadable_Time()).append("]").toString());

                            fw.write(nl);
                        }
                        fw.write(new StringBuilder().append("[").append(recv.toString()).append("]").append(slea.toString(true)).toString());
                        fw.write(nl);
                        fw.flush();
                    }
                    ...
                    if (!ServerConstants.Use_Fixed_IV) {
                        FileoutputUtil.outputFileError("log\\Packet_Except.log", e);
                        FileoutputUtil.log("log\\Packet_Except.log", new StringBuilder().append("Packet: ").append(header_num).append("\r\n").append(slea.toString(true)).toString());
                    }
        ...
        if (c.isLocalhost()) {
            ...
            sb.append(HexTool.toString((byte[]) (byte[]) message)).append("\r\n").append(HexTool.toStringFromAscii((byte[]) (byte[]) message));
            System.out.println(sb.toString());
        }
    }

    public static void handlePacket(RecvPacketOpcode header, LittleEndianAccessor slea, MapleClient c, boolean cs) throws Exception {
        switch (header) {
            case PONG:
                c.pongReceived();
                break;
 ...
 //               PartyHandler.Expedition(slea, c);
            ...
            case USE_UNK_EFFECT:
                break;
            default:
                System.out.println(new StringBuilder().append("[没有处理的] Recv [").append(header.toString()).append("] 发现").toString());
        }
    }
    ...
    private static class LoggedPacket {
        ...
        public String toString() {
            StringBuilder sb = new StringBuilder();
            sb.append("[IP: ").append(this.ip).append("] [").append(this.accId).append('|').append(this.accName).append('|').append(this.chrName).append("] [").append(this.npcId).append('|').append(this.mapId).append("] [Time: ").append(this.timestamp).append("] [").append(this.time).append(']');
            sb.append(nl);
            sb.append("[Op: ").append(this.op).append("] [").append(this.packet).append(']');
            return sb.toString();
        }
    }
}
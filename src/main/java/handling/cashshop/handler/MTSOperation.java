...
public class MTSOperation {
    ...
    public static void MTSOperation(LittleEndianAccessor slea, MapleClient c) {
        ...
        if (op == 2) {
            ...
            if ((ii.isCash(itemid)) || (quantity <= 0) || (item == null) || (item.getQuantity() <= 0) || (item.getItemId() != itemid) || (item.getUniqueId() > 0) || (item.getQuantity() < quantity) || (price < 100) || (c.getPlayer().getMeso() < 10000) || (cart.getNotYetSold().size() >= 10) || (ii.isDropRestricted(itemid)) || (ii.isAccountShared(itemid)) || (item.getExpiration() > -1L) || (item.getFlag() > 0)) {
                c.getSession().write(MTSCSPacket.getMTSFailSell());
                doMTSPackets(cart, c);
                return;
            }
            ...
            c.getPlayer().gainMeso(-10000, false);

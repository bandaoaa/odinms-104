                ...
                if (itemId == 5060000) {
                    ...
                    if (chr.getLevel() < 10) {
                        ...
                        if (item != null && chr.getInventory(MapleInventoryType.USE).getNumFreeSlot() >= 1) {
                            ...
                            if ((eq.getState() >= 17) && (eq.getState() != 20) && (!ii.isCash(eq.getItemId()))) {
                                int 保护位置 = 0;
                                int 保护潜能ID = 0;
                                if (slea.available() >= 4L) {
                                    保护位置 = slea.readInt();
                                    保护潜能ID = slea.readShort();
                                }
                                if (chr.isAdmin()) {
                                    chr.dropMessage(-11, new StringBuilder().append("保护位置 ").append(保护位置).append("  保护潜能ID ").append(保护潜能ID).toString());
                                }

                                eq.renewPotentials(保护位置, 保护潜能ID, 0, chr.isAdmin() ? 10 * stateRate : stateRate, chr.鉴定概率("B级概率"), chr.鉴定概率("A级概率"), chr.鉴定概率("S级概率"), chr.鉴定概率("SS级概率"), chr.鉴定概率("潜能等级概率2"), chr.鉴定概率("潜能等级概率3"), chr.鉴定概率("潜能等级概率4"), chr.鉴定概率("潜能等级概率5"));

                                //   eq.renewPotential(0, chr.isAdmin() ? 99 : stateRate);
                                // c.getSession().write(MaplePacketCreator.scrolledItem(toUse, item, false, true));
                                c.getSession().write(MaplePacketCreator.scrolledItem(toUse, item, false, false));
                    ...
                    if (chr.getLevel() < 30) {
                        chr.dropMessage(1, "使用这个道具需要等级达到30级.");
                    } else {
                        Item item = chr.getInventory(MapleInventoryType.EQUIP).getItem((short) (byte) slea.readInt());
                        if ((item != null) && (chr.getInventory(MapleInventoryType.USE).getNumFreeSlot() >= 1)) {
                            Equip eq = (Equip) item;
                            ...
                            int stateRate = chr.get潜能数目改变概率();
                            if ((eq.getState() >= 17) && (eq.getState() != 20) && (!ii.isCash(eq.getItemId()))) {
                                eq.renewPotentials(0, 0, 1, chr.isAdmin() ? 10 * stateRate : stateRate, chr.鉴定概率("B级概率"), chr.鉴定概率("A级概率"), chr.鉴定概率("S级概率"), chr.鉴定概率("SS级概率"), chr.鉴定概率("潜能等级概率2"), chr.鉴定概率("潜能等级概率3"), chr.鉴定概率("潜能等级概率4"), chr.鉴定概率("潜能等级概率5"));
                                //    if ((eq.getState() >= 17) && (eq.getState() != 20)) {
                    ...
                    if (chr.getLevel() < 50) {
                        ...
                        if ((item != null) && (chr.getInventory(MapleInventoryType.USE).getNumFreeSlot() >= 1)) {
                            ...
                            // int stateRate = chr.getClient().getChannelServer().getStateRate();
                            int stateRate = chr.get潜能数目改变概率();
                            if ((eq.getState() >= 17) && (!ii.isCash(eq.getItemId()))) {
                                //     if ((eq.getState() >= 17) && (!ii.isCash(eq.getItemId()))) {
                                eq.renewPotentials(0, 0, 2, chr.isAdmin() ? 10 * stateRate : stateRate, chr.鉴定概率("B级概率"), chr.鉴定概率("A级概率"), chr.鉴定概率("S级概率"), chr.鉴定概率("SS级概率"), chr.鉴定概率("潜能等级概率2"), chr.鉴定概率("潜能等级概率3"), chr.鉴定概率("潜能等级概率4"), chr.鉴定概率("潜能等级概率5"));

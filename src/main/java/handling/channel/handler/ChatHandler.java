...
//                log.info("[信息] " + chr.getName() + " : " + text);
                if ((chr.getCanTalk()) || (chr.isStaff())) {
                    ...
                    c.getSession().write(MaplePacketCreator.serverNotice(6, "你因为废话太多了，所以被禁言了."));
                }
          }
        }
    }
    ...
    public static void Others(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        ...
        if ((chr == null) || (!chr.getCanTalk())) {
            c.getSession().write(MaplePacketCreator.serverNotice(6, "你被禁言了"));
            return;
        }
        ...
        if (c.isMonitored()) {
            ...
            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM 信息] " + MapleCharacterUtil.makeMapleReadable(chr.getName()) + " 说 (" + chattype + "): " + chattext));
        }
        ...
        switch (mode) {
            ...
            case 6:
                ...
                if (!c.getPlayer().getCanTalk()) {
                    c.getSession().write(MaplePacketCreator.serverNotice(6, "你已经被静音，因此无法说话。"));
                    return;
                }
                ...
                if (ch > 0) {
                    ...
                    if (c.isMonitored()) {
                        World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, c.getPlayer().getName() + " 低声说 " + recipient + " : " + text));

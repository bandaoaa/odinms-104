...
public class MapleGuildAlliance implements Serializable {
    ...
    private static final Logger log = Logger.getLogger(MapleGuildAlliance.class);
    ...
    public boolean setLeaderId(int c, boolean sameGuild) {
        ...
        for (int i = 0; i < getNoGuilds(); i++) {
            MapleGuild g_ = Guild.getGuild(this.guilds[i]);
            if (g_ != null) {
        ...
        broadcast(GuildPacket.changeAllianceLeader(this.allianceid, this.leaderid, c));
        broadcast(GuildPacket.updateAllianceLeader(this.allianceid, this.leaderid, c));
        broadcast(GuildPacket.getAllianceUpdate(this));
        broadcast(GuildPacket.getGuildAlliance(this));
        this.leaderid = c;
        saveToDb();

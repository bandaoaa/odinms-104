...
public class MapleItemInformationProvider {
    ...
    public void runEtc() {
        ...
        for (MapleData dat : potsData) {
            ...
            this.potentialCache.put(Integer.valueOf(Integer.parseInt(dat.getName())), items);
        }

        Map gradeS = new HashMap();
        Map gradeA = new HashMap();
        Map gradeB = new HashMap();
        Map gradeC = new HashMap();
        Map gradeD = new HashMap();
        ...
        for (MapleData dat : nebuliteData) {
            ...
            switch (GameConstants.getNebuliteGrade(item.opID)) {
                case 4:
                    gradeS.put(Integer.valueOf(Integer.parseInt(dat.getName())), item);
                    break;
                case 3:
                    gradeA.put(Integer.valueOf(Integer.parseInt(dat.getName())), item);
                    break;
                case 2:
                    gradeB.put(Integer.valueOf(Integer.parseInt(dat.getName())), item);
                    break;
                case 1:
                    gradeC.put(Integer.valueOf(Integer.parseInt(dat.getName())), item);
                    break;
                case 0:
                    gradeD.put(Integer.valueOf(Integer.parseInt(dat.getName())), item);
            }
        }

        this.socketCache.put(Integer.valueOf(4), gradeS);
        this.socketCache.put(Integer.valueOf(3), gradeA);
        this.socketCache.put(Integer.valueOf(2), gradeB);
        this.socketCache.put(Integer.valueOf(1), gradeC);
        this.socketCache.put(Integer.valueOf(0), gradeD);
        ...
        for (MapleDataEntry d : e.getFiles()) {
            ...
            this.androids.put(Integer.valueOf(Integer.parseInt(d.getName().substring(0, 4))), new Pair(hair, face));
        }
    ...
    public Item scrollEquipWithId(Item equip, Item scrollId, boolean ws, MapleCharacter chr, int vegas) {
        if (equip.getType() == 1) {
            ...
            if ((GameConstants.is潜能卷轴(scrollId.getItemId())) || (GameConstants.is强化卷轴(scrollId.getItemId())) || (GameConstants.is特殊卷轴(scrollId.getItemId())) || (Randomizer.nextInt(100) <= success)) {
                switch (scrollId.getItemId()) {
                    ...
                    default:
                        if (GameConstants.is枫叶卷轴(scrollId.getItemId())) {
                            ...
                            for (int i = 0; i < (scrollId.getItemId() == 2049304 ? 3 : scrollId.getItemId() == 2049305 ? 4 : 1); i++) {
                ...
                if ((!GameConstants.is白医卷轴(scrollId.getItemId())) && (!GameConstants.is特殊卷轴(scrollId.getItemId())) && (!GameConstants.is强化卷轴(scrollId.getItemId())) && (!GameConstants.is潜能卷轴(scrollId.getItemId()))) {
                ...
                if ((!ws) && (!GameConstants.is白医卷轴(scrollId.getItemId())) && (!GameConstants.is特殊卷轴(scrollId.getItemId())) && (!GameConstants.is强化卷轴(scrollId.getItemId())) && (!GameConstants.is潜能卷轴(scrollId.getItemId()))) {
                    short oldFlag = nEquip.getFlag();
                    if (ItemFlag.保护卷轴.check(oldFlag)) {
                        nEquip.setFlag((short) (oldFlag - ItemFlag.保护卷轴.getValue()));
                    } else {

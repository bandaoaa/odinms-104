...
public final class MapleMap {
  ...
  public Point 掉落坐标(Point initial, Point fallback) {
        Point ret = calcPointBelow(new Point(initial.x+35, initial.y - 50));
        if (ret == null) {
            return fallback;
        }
        return ret;
...
//                int tempmeso = Math.min(30000, (int) (cmServerrate * (mob.getMobExp()) * (1.0 + Math.random() * 20) / 10.0));
//                       int mesos = Randomizer.nextInt(1 + Math.abs(de.Maximum)) + de.Minimum;
//                    System.out.println("itemid==0 || meso"+mesos);
                    if (mesos > 0) {
                    ...
                    if (GameConstants.isNoticeItem(de.itemId)) {
                        broadcastMessage(MaplePacketCreator.serverNotice(6, new StringBuilder().append("[掉宝提示] 玩家 ").append(chr.getName()).append(" 在 ").append(chr.getMap().getMapName()).append(" 杀死 ").append(mob.getStats().getName()).append(" 掉落道具 ").append(ii.getName(de.itemId)).toString()));
                    }
    ...
    public final void killMonster(final MapleMonster monster, final MapleCharacter chr, boolean withDrops, boolean second, byte animation, int lastSkill) {
        ...
        if ((withDrops) && (dropOwner != 1)) {
            ...
            toDrop = new Item(itemid, (short) 0, (short)1, (short) 0);
            spawnItemDrop2(monster, chr, toDrop, monster.getPosition(), true, true);
            }
           
            
            //(final MapleMapObject dropper, final MapleCharacter owner, Item item, Point pos, boolean ffaDrop, boolean playerDrop) {
    ...
    public final void spawnRandDrop() { //市场掉i装备
...
public final void spawnItemDrop2(final MapleMapObject dropper, final MapleCharacter owner, Item item, Point pos, boolean ffaDrop, boolean playerDrop) {
        final Point droppos = 掉落坐标(pos, pos);
        final MapleMapItem drop = new MapleMapItem(item, droppos, dropper, owner, (byte) 2, playerDrop);

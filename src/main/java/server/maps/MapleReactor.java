...
public class MapleReactor extends MapleMapObject {
    ...
    public void delayedDestroyReactor(long delay) {
        MapTimer.getInstance().schedule(new Runnable() {
            ...
            @Override
            public void run() {
                MapleReactor.this.map.destroyReactor(getObjectId());
            }
        }, delay);
    }
    ...
    public String toString() {
        return "反应堆 工作ID:" + getObjectId() + " ReactorID: " + this.rid + " 坐标: " + getPosition().toString() + " 状态: " + this.state + " 类型: " + this.stats.getType(this.state);
    }
